...
document.addEventListener("contextmenu",function(risfat) {

    risfat.preventDefault();
}
...
if ($this->session->userdata('login_status') != '1' && $subscriptions != "TRUE" ) {
    echo $this->common_model->get_ads('player_top');
...
ovoo_player.dock({
        title: '<?php echo $watch_videos->title;?>',
        description: '<?php echo $season['seasons_name']; ?>'
      });
...
foreach ($related_videos as $count):
...
function getTitle(){
      ...
      function next(){
        ...
        window.location.href = "<?php echo base_url('watch/'.$videos['slug'].'.html');?>";
      }
                ...
                <?php if($this->common_model->get_ads_status('player_bottom')=='1'): ?>
                    ...
                    <div class="row">
                        <div class="col-md-12 text-center m-b-10">
                            <?php
if ($this->session->userdata('login_status') != '1' && $subscriptions != "TRUE" ) {
    echo $this->common_model->get_ads('player_bottom');
}elseif ($this->session->userdata('login_status') == '1' && $subscriptions != "TRUE" ) {
    echo $this->common_model->get_ads('player_bottom');
}
?>
                ...
                <div class="col-md-3 col-sm-6">
                    <div class="sidebar">
                        <div class="ad_300x250 m-b-10">
                            <?php
if ($this->session->userdata('login_status') != '1' && $subscriptions != "TRUE" ) {
    echo $this->common_model->get_ads('sidebar');
}elseif ($this->session->userdata('login_status') == '1' && $subscriptions != "TRUE" ) {
    echo $this->common_model->get_ads('sidebar');
}
?>  
    ...
    function wish_list_add(list_type = '', videos_id = '') {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (status == "success") {
                    swal('Success!', '<?php echo $watch_videos->title;?> Added to Your ' + list_name + ' List.', 'success');
                } else if (status == "login_fail") {
                    swal('OPPS!', 'Please Login to Add your ' + list_name + ' List.', 'error');
                } else {
                    swal('OPPS!', '<?php echo $watch_videos->title;?> Already exist On Your ' + list_name + ' list.', 'error');
                }
            }
        });
    }
