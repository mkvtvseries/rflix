...
class User extends Home_Core_Controller{
        ...
        if ($param1 == 'do_signup') {
            ...
            if($registration_enable =="1"):
                ...
                if ($this->form_validation->run() == FALSE):
                    ...
                    if($email_exist):
                        $this->session->set_flashdata('error', 'Signup failed. Email is already Exist');                        
                    else:
                        $data['join_date']       = date('Y-m-d H:i:s');
                        $data['last_login']       = date('Y-m-d H:i:s');
                        $this->db->insert('user', $data);
                        ...
                        $this->session->set_flashdata('success', 'Signup Successful. Now you can Login to our System');                    
                        redirect(base_url() . 'user/login', 'refresh');
                    endif;
                endif;
            else:
    ...
    function change_password($param1 = '', $param2 = ''){
        ...
        if ($param1 == 'update') {
            ...
            elseif ($current_password !=$password ){
                $this->session->set_flashdata('error', 'Old password Did Mot Matched.');
    ...
    function do_login(){
        ...
        if ($this->form_validation->run() == FALSE):
            ...
            redirect(base_url() . 'user/profile', 'refresh');
                ...
                $this->session->set_flashdata('login_error', 'Username & Password did not matched..');
                redirect(base_url() . 'user/login', 'refresh');
            endif;
        endif;      
    }
    ...
    function subscription($param1 = '', $param2 = ''){
        ...
        if ($param1 == 'update') {
            ...
            $this->db->update('user', $data);
            move_uploaded_file($_FILES['photo']['tmp_name'], 'uploads/user_image/' .$user_id.'.jpg');            
            $this->session->set_flashdata('success', 'Profile Information has Been Updated.');
            redirect(base_url() . 'user/update_profile/', 'refresh');
        }
